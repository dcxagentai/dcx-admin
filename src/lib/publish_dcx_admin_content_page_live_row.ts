/**
 * CONTEXT:
 * This file publishes one admin content-page live row through the backend immutable version model.
 *
 * CODE:
 */
...
export async function publishDcxAdminContentPageLiveRow(params: {
  apiBaseUrl: string
  pageId: number
  categoryKey: string
  pageTitle: string
  ...
  const response = await fetch(new URL("/admin/content/pages/publish-live-row", params.apiBaseUrl), {
    method: "POST",
    credentials: "include",
  ...
  if (!response.ok || payload.ok !== true) {
    const errorPayload =
      payload && payload.ok === false
        ? payload.error
        : {
            code: "DCX_ADMIN_CONTENT_PAGE_PUBLISH_FAILED",
