/**
 * CONTEXT:
 * This file autosaves one admin content-page live row through the backend immutable version model.
 *
 * CODE:
 */
...
export async function saveDcxAdminContentPageLiveRow(params: {
  apiBaseUrl: string
  pageId: number
  categoryKey: string
  pageTitle: string
  ...
  publishedAtTsMs: number | null
}): Promise<SuccessResponse> {
  const response = await fetch(new URL("/admin/content/pages/save-live-row", params.apiBaseUrl), {
    method: "POST",
    credentials: "include",
    ...
    body: JSON.stringify({
      ...
      published_at_ts_ms: params.publishedAtTsMs,
    }),
  })
  ...
  if (!response.ok || payload.ok !== true) {
    const errorPayload =
      payload && payload.ok === false
        ? payload.error
        : {
            code: "DCX_ADMIN_CONTENT_PAGE_SAVE_FAILED",
            message: "We could not save that DCX content page.",
